...
Перед изменением в этом репозитории рекомендуется добровольно запустить
предварительную проверку Ядра:

```bash
node scripts/preflight.mjs
```

Отсутствие запуска означает `UNVERIFIED` и не блокирует работу (§11). Обычный
запуск проверяет только самодостаточность Kernel и не требует
`MERIDIAN_INSTANCE` (§10): разработка самого Meridian не зависит от того,
подключён ли какой-либо Instance. Явно запущенная команда обязана честно
сообщить устаревший Kernel ненулевым кодом; решение продолжить остаётся в
пределах назначения и текущей owner-policy (§11).
...
Переданный владельцем вывод добровольных проверок считается входом проверки
результата и не требует автоматического повторного локального прогона
архитектором или интегратором Git. Отсутствующий результат отмечается как
`UNVERIFIED` и сам по себе не блокирует вердикт (§11).
...
1. подтвердить точную принятую исходную ветку, границы
...
4. подтвердить достижимость пакетного коммита из целевой линии, наличие
   ...
   записать добровольные послеслияционные проверки как `OK`, `FAIL` либо
   `UNVERIFIED` без блокировки по факту их отсутствия;
...
<!-- meridian:begin instruction-section id=optional-kernel-verification owner=workspace-owner generated=no -->
## 11. Решение владельца: проверки Kernel добровольны

#### 2026-09-26

Эта датированная запись имеет приоритет над более ранними формулировками этого
документа и активных планов об обязательных preflight, тестах, validators,
conformance, smoke, CI, pre-/post-merge и release gates для **самого Kernel**.

- Все такие команды сохранены и рекомендуются как добровольное доказательство,
  но ни одна из них не является условием начала работы, архитектурной приёмки,
  Git-интеграции, merge, выпуска или подключения Concord.
- `node scripts/preflight.mjs` перед изменением рекомендуется, но его отсутствие
  или отрицательный результат не лишает участника полномочий продолжить в
  пределах назначения. Явно запущенная команда по-прежнему сообщает настоящий
  результат и не маскирует ошибку.
- Отсутствующий результат записывается как `UNVERIFIED`; это состояние не равно
  `OK`, но само по себе не блокирует `ACCEPTED`, интеграцию или переход.
- Архитектор оценивает содержание и предъявленные evidence; он не обязан
  запускать проверки. Интегратор сохраняет owner-managed MR/merge-модель, но не
  требует status check или pre-/post-merge test result.
- `hooks/pre-push` по умолчанию пропускает проверки. Полный сохранённый набор
  включается явно через `MERIDIAN_RUN_OPTIONAL_CHECKS=1`; в opt-in режиме
  реальный сбой возвращает ненулевой код.
- Физическое архивирование прежнего Instance и наличие либо отсутствие
  `MERIDIAN_INSTANCE` не блокируют Kernel, выпуск 0.7.0 или Concord. Прежний
  Instance остаётся необязательным историческим/миграционным источником.

Эта политика не ослабляет production CLI, validators или тесты при их явном
вызове и не делает product-specific проверки автоматически необязательными:
продукт определяет их в своём собственном tracked-контуре.
<!-- meridian:end instruction-section id=optional-kernel-verification -->
