...
- **Объединённая квалификация совместимости рабочих пространств
  ...
  - **Ссылки, а не встроенные записи — все три с обязательным `sha256`.**
    `payload` не встраивает ни один из трёх составляемых пакетов целиком:
    `workspace_connection_ref` (всегда обязателен), `migration_plan_ref` и
    `canonical_export_ref` (оба допускают `null`) — одна закрытая форма
    `pinned_ref` (`{record_type, id, reference, sha256}`) с **обязательным**
    `sha256` на всех трёх, разрешаемые через внешнюю границу **внутри**
    проверки и никогда не персистируемые: обнаруженные источники, кандидаты
    правил и их `raw_excerpt`/`normalized_text` не достигают самой записи
    квалификации. `sha256` пинует фактически пересчитанный дайджест
    ...
    `computeConnectionDigest`, для сканирования подключения (у
    ...
    себе не доказывает тождество версии; доказано тремя самостоятельными
    негативными тестами подмены содержимого, по одному на каждую ссылку.
  ...
  - **Композиция, а не копия формы.** Разрешённый `workspace_connection_ref` —
    полная запись `existing-project-compatibility-mode`, проверяемая вызовом
    **той же** `evaluateExistingProjectCompatibilityMode`; разрешённые
  ...
  - **Согласованность области.** `scope` записи квалификации обязан полностью
    совпадать со `scope` каждой из **разрешённых** записей; разрешённый
  ...
  - **Закрытая матрица решений — не то же утверждение, что приёмочные
    ...
    пересчитываемый `next_step` разрешённого сканирования, наличие
    нерешённого кандидата правила и, когда план разрешается, его собственный
    ...
    находкой на пустом документе. Конфликт кандидатов проверен в ОБОИХ
    формах — разрешённый владельцем (один `accepted`, другой
