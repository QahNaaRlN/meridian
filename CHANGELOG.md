...
- **Корректирующий пакет `knowledge-agent-foundation` начат: роли баз, эволюция
  ...
  реализованный модуль границы будущего Knowledge Resolver. Реализация
  подготовлена в рабочем дереве, ожидает независимой проверки и
  Git-интеграции — это не запись о приёмке пакета
...
- **Прикладное разрешение норм Rust (`rust-rule-resolution`) — пакет 5
  ...
  принимает закрытый набор уже загруженных значений и явно переданные схемы,
  проверяет полный envelope применимости, преобразует транспортные данные в
  строгие типы `meridian-core`, вызывает чистый `resolve_rules`, сериализует и
  повторно проверяет результат. Неизвестные поля и несовместимые сочетания
  отклоняются; порядок ключей транспортного JSON не используется как
  предметный сигнал. Отсутствие `protocol_routes[].mandatory` нормализуется в
  `false` — как в действующем Node.js-эталоне — а не отклоняется как
  невалидный запрос; явно переданное значение обязано быть boolean.
  Внутренний ключ выбора победителя среди маршрутов одного протокола и
  одного `source.rank()` при равенстве вычисляется из фиксированного
  канонического порядка полей самого маршрута (`mandatory` входит в него
  всегда, даже если поле отсутствовало во входе), а не из порядка ключей,
  в котором их перечислил транспортный JSON, — осознанное усиление границы,
  закреплённое отдельными модульными проверками. Файловый, Git, сетевой,
  env-, DB-, process- и CLI-ввод-вывод не добавлены. Общий состязательный
  корпус запускает настоящие Node.js- и Rust-производители на шести
  сценариях (пустой источник, применимая Kernel-норма с маршрутом и
  повторным разрешением, недекомпозированная инициатива, repository-норма с
  точным intake-происхождением, маршрут без `mandatory`, и два маршрута
  одного протокола и одного `source.rank()`, различающиеся provenance,
  проверяющие реальный детерминированный выбор победителя) через тот же
  независимый conformance harness, что и пакет 4.
...
- **Предметное ядро Rust (`rust-domain-core`) — пакет 3 программы
  ...
  ядро без побочных эффектов: строгие предметные типы (`SemanticId`,
  `WorkspaceId`, `RepositoryId`, `Revision`, `ContentDigest`, `EvidenceRef`,
  `Scope`, `Origin`, `Authority`, `Verdict`, `Diagnostic` —
  `meridian-rust-target-architecture.md` §5), чистый resolver (`resolveRules`
  из `scripts/rule-resolver.mjs`, перенесённый с сохранением потока
  управления и семантики каждого правила — без файлового ввода-вывода,
  разбора аргументов, переменных окружения, JSON/YAML и `main()`), структуры
  и проверки плана миграции Экземпляра (девять свойств
  `instance-data-migration.md`), и структуры доказательств с правилами
  связывания утверждения с проверяемым результатом
  (`evidence-and-handoff-contract.md` §5, предметное подмножество без полного
  контракта передачи). Каждый строгий тип валидируется в публичном
  конструкторе; закрытые множества выражены через `enum`, а не свободные
  строки; условно обязательные поля выражены вариантами `enum`, а не
  отдельной рантайм-проверкой поверх плоской структуры. `#![forbid(unsafe_code)]`
  сохранён; `meridian-core` не зависит ни от одного другого локального крейта
  workspace (`cargo tree -p meridian-core` — только `sha2`, `ryu` и их транзитивные
  зависимости).
  - **Побайтовая совместимость `plan_fingerprint`/`idempotency_key` с
    Node-эталоном.** `meridian-core/src/migration/canonical.rs` строит ту же
    каноническую JSON-проекцию, что и `computePlanFingerprint`/
    `computeIdempotencyKey` (`scripts/lib/instance-data-migration.mjs`): те
    же имена полей, тот же порядок ключей (рекурсивно отсортированный внутри
    каждого канонизируемого объекта; буквальный для двух внешних
    JS-объектных литералов `{source, record_units, mappings, rollback}` и
    `{scope, repository_ref, revision, digest}`, которые Node строит без
    прогона через `canonicalize()`), то же правило экранирования строк
    JSON.stringify, те же условно присутствующие поля (`qualification_reason`,
    `deterministic_plan_ref`/`restoration_evidence_ref` и т. д.). Реализовано
    через `meridian-core/src/json.rs` — минимальный приватный JSON
    writer/reader, общий для `migration` и `resolver` (ниже); он не
    экспортируется из крейта, не принимает произвольный транспортный
    документ и не является тем «транспортным JSON-парсером», который
    архитектурно запрещён `meridian-core`. Совместимость доказана пятью
    эталонными тестами (`meridian-core/tests/migration_fingerprint_reference.rs`),
    сравнивающими вывод `compute_plan_fingerprint`/`compute_idempotency_key`
    с хешами, фактически вычисленными вызовом настоящих
    `computePlanFingerprint`/`computeIdempotencyKey` из
    `scripts/lib/instance-data-migration.mjs` на эквивалентных входных
    данных — не только с внутренней детерминированностью Rust-реализации.
  - **Семантическая проверка `ContentEnvelope`, включая полную каноничность
    JSON-содержимого (`checkContentEnvelope`, `instance-data-migration.md`
    §4.1).** `ContentEnvelope::new` — единственный конструктор, независимо
    пересчитывающий SHA-256 от фактических представленных байт (UTF-8 байт
    `content` при `encoding: utf-8`, декодированных бинарных байт при
    `encoding: base64`) и отклоняющий несовпадение с заявленным `digest`;
    строгое каноническое декодирование Base64
    (`meridian-core/src/migration/base64.rs`, round-trip
    decode-then-re-encode, отклоняющий не только неверный
    алфавит/выравнивание, но и синтаксически допустимую, но неканоническую
    запись); и проверку каноничности JSON-содержимого для `media_type`
    `application/json`/`*+json` через `crate::json::is_canonical`. Нет пути
    сконструировать `ContentEnvelope`, закрытый по форме, но не проверенный
    по существу, — единственный конструктор выполняет обе проверки
    безусловно. `crate::json` (`meridian-core/src/json.rs`) реализует
    `JSON.stringify(canonicalize(JSON.parse(content)))`: дробные и
    экспоненциальные числа парсятся в `f64` и переформатируются через
    прямой перенос правил форматирования ECMA-262 `Number::toString` (тот
    же регистровый выбор целое/десятичное/ведущие нули/экспоненциальное
    представление, что и `JSON.stringify`); повторяющиеся ключи
    обрабатываются по семантике `JSON.parse` — остаётся последнее
    значение, поэтому текст с дублирующимся ключом никогда не считается
    каноническим; сортировка ключей объекта — по последовательности
    кодовых единиц UTF-16 (`str::encode_utf16`), как у
    `Array.prototype.sort()`/`Object.keys(value).sort()`, а не по
    Unicode scalar value, как дал бы порядок `str`/`char` в Rust
    (расхождение проявляется на ключах вне BMP — суррогатная пара такого
    ключа начинается с кодовой единицы `0xD800..=0xDBFF`, которая МЕНЬШЕ
    диапазона `0xE000..=0xFFFF`, хотя как scalar value больше). Совместимость
    доказана 20 эталонными тестами
    (`meridian-core/tests/json_canonical_reference.rs`) против фактического
    результата настоящего `checkContentEnvelope`
    (`scripts/lib/instance-data-migration.mjs`) — включая `{"a":1.5}`,
    каноническую и неканоническую экспоненциальную запись, дублирующийся
    ключ, границы формата (`1e20`/`1e21`, `1e-6`/`1e-7`, `5e-324`,
    `1.7976931348623157e+308`), `-0`/`-0.0`, классический артефакт
    `0.1+0.2`, порядок ключей `U+E000`/`U+10000`, точку округления до
    чётного (`-86666878403640.62`) и одиночные/парные суррогаты строковых
    значений — см. следующий пункт для истории и границ этой проверки.
  - **Исправление (раунд 4): источник цифр `Number::toString` и модель
    строк JSON — обе ошибки найдены независимым исполняемым
    дифференциальным сравнением, не заявлены заранее как исправленные.**
    Предыдущая ревизия этого пакета (раунд 3) утверждала «полную
    каноничность JSON-содержимого» и «прямой перенос» `Number::toString`;
    оба утверждения оказались неточными для двух категорий входа:
    - **Числа.** Источником цифр служило форматирование Rust `{:e}`
      (`LowerExp`) — оно тоже даёт кратчайшую round-trip десятичную
      строку, но НЕ реализует правило ECMA-262 «при точной десятичной
      середине выбрать чётную цифру». Контрпример:
      `-86666878403640.625` — точная середина между `...62` и `...63`;
      реальный `JSON.stringify` (V8, следуя спецификации) даёт чётное
      `...62`, а Rust `{:e}` — нечётное `...63`, поэтому текст
      `{"n":-86666878403640.62}`, произведённый настоящим
      `JSON.stringify` и принимаемый настоящим `checkContentEnvelope`,
      прошлая ревизия отклоняла как `NotCanonicalJson`. Источник цифр
      заменён на крейт `ryu` (см. ниже); окружающий выбор регистра
      A/B/C/D и пунктуация ECMA-262 не менялись. Совместимость проверена
      не «несколькими вручную выбранными граничными примерами», а
      детерминированным дифференциальным корпусом из 6201 значения
      (`meridian-core/src/json_number_corpus_data.rs`, порождён из сырых
      битовых представлений `f64` — 6000 равномерно случайных плюс точка
      округления и её ближайшие соседи, границы степеней десяти от 1e-30
      до 1e30 и границы субнормального диапазона), для каждого значения
      сверенным с фактическим выводом настоящего `JSON.stringify`
      (`meridian-core/src/json.rs`, тест
      `matches_the_node_reference_for_a_large_deterministic_bit_pattern_corpus`,
      и повторно через публичный `ContentEnvelope`
      в `meridian-core/tests/json_number_corpus_reference.rs`). Это
      сильное эмпирическое подтверждение на широком корпусе, а не
      математическое доказательство совпадения для всех ~2^64 конечных
      значений `f64` — модуль не заявляет безусловную точную
      эквивалентность.
    - **Строки.** `Parser::parse_string` отклонял одиночный
      (непарный) суррогат — Rust `String`/`char` физически не может его
      хранить. Контрпример: `{"a":"\ud800"}` — канонический текст
      настоящего `JSON.stringify`, отклонявшийся прошлой ревизией.
      Внутреннее представление содержимого JSON-строки заменено с Rust
      `String` на последовательность кодовых единиц UTF-16 (`Vec<u16>`);
      `\uXXXX` теперь всегда означает ровно одну кодовую единицу без
      проверки диапазона/парности при разборе; `write_json_string`
      воспроизводит `QuoteJSONString`: корректная пара high+low
      суррогатов сшивается и выводится как буквальный astral-символ, а
      одиночный суррогат экранируется индивидуально как `\uXXXX` — оба
      правила подтверждены против настоящего вывода `JSON.stringify`.
      На этом этапе (раунд 4) ключи объектов/литералов ещё оставались
      `String`, так что документ с одиночным суррогатом в ключе всё ещё
      отклонялся — эта оставшаяся неточность найдена и закрыта раундом 5
      ниже (в ключе одиночный суррогат `String` физически хранить не может,
      но это не делает такой ключ «намеренно» неканоническим:
      `checkContentEnvelope` его принимает). Проверено
      пятью отдельными сценариями (одиночный высокий/низкий суррогат,
      корректная пара, соседние одиночные суррогаты, каноническое
      повторное экранирование пары, записанной двумя escape-
      последовательностями вместо буквального символа) — как модульными
      тестами в `meridian-core/src/json.rs`, так и эталонными тестами
      против настоящего `checkContentEnvelope` в
      `meridian-core/tests/json_canonical_reference.rs`.
    - Уже исправленное в раунде 3 (семантическая проверка digest/Base64,
      обработка повторяющихся ключей, сортировка ключей по UTF-16,
      атомарный `ProtocolRoute` и вычисляемый ключ разрешения равенства,
      `distinctMandatory.length`, эталонные отпечатки плана) не
      откатывалось этим исправлением.
  - **Исправление (раунд 5): ключи разобранных объектов используют ту же
    UTF-16-модель, что и значения строк.** Раунд 4 перенёс содержимое
    JSON-строкового ЗНАЧЕНИЯ на `Vec<u16>` (см. выше), но `Parser::parse_object`
    по-прежнему конвертировал разобранный ключ в Rust `String` через
    `String::from_utf16(...).ok()?` — для ключа с одиночным суррогатом это
    всегда `None`, так что документ отклонялся. Контрпример:
    `{"\ud800":1}` — канонический текст настоящего `checkContentEnvelope`
    (JS-объекты индексируются строками UTF-16, не Unicode scalar value, и
    ключ ничем не отличается от значения в этом отношении), отклонявшийся
    прошлой ревизией как `NotCanonicalJson`. `Json::Object` теперь хранит
    `Vec<(Vec<u16>, Json)>` вместо `Vec<(String, Json)>` — тот же тип
    ключа, что и содержимое `Json::Str`, а не отдельное «безопасное»
    представление; `Parser::parse_object` берёт `Vec<u16>` напрямую из
    `Parser::parse_string`, без промежуточного `String`. Из этого без
    дополнительных изменений следуют: сравнение повторяющихся ключей (в
    т.ч. дублирующегося одиночного суррогата) — по последовательности
    кодовых единиц `Vec<u16>` с семантикой `JSON.parse` «последнее
    значение побеждает»; сортировка ключей при записи — собственный `Ord`
    `Vec<u16>` (лексикографический по `u16`), совпадающий с
    `Object.keys(value).sort()` и для ключей с суррогатами (но, как
    выяснилось раундом 6 ниже, ОДНОЙ этой лексикографической сортировки
    недостаточно для итогового порядка ключей в тексте — это исправление
    раунда 5 остаётся корректным для не-числовых ключей, просто неполным
    в границах, которые раунд 6 закрывает отдельно). `QuoteJSONString`
    для ключей — тот же `write_json_string`, что и для значений (корректная
    пара суррогатов сшивается в буквальный astral-символ, одиночный
    суррогат экранируется индивидуально), без отдельной `write_json_key_string`
    для `Json::Object` (эта функция сохранена только для `Json::Literal`,
    чьи ключи — заведомо известные во время компиляции `&'static str` и
    никогда не содержат одиночный суррогат). `crate::migration::canonical`
    (единственный вызывающий код, строящий `Json::Object` из Rust-строк, а
    не из разбора) обновлён на новый конструктор `Json::key(&str) ->
    Vec<u16>` — аналог `Json::str` для ключей. Совместимость доказана
    девятью новыми эталонными тестами через публичный `ContentEnvelope`
    против фактического результата настоящего `checkContentEnvelope`
    (`meridian-core/tests/json_canonical_reference.rs`): одиночный
    высокий/низкий суррогат в ключе принят; корректная пара суррогатов в
    ключе как буквальный astral-символ принята, та же пара, записанная
    двумя escape-последовательностями, отклонена; повторяющийся одиночный
    суррогат-ключ — исходный текст с дублем отклонён, уже свёрнутая форма
    с последним значением принята; порядок сортировки одиночного
    суррогата относительно обычного BMP-ключа и относительно astral-ключа,
    делящего с ним первую кодовую единицу, совпадает с Node (короткая
    последовательность, совпадающая по общему префиксу, сортируется
    раньше — то же правило префикса, что использует и сравнение строк
    JS). Модульный тест, закреплявший ошибочное отклонение суррогата в
    ключе, заменён положительной проверкой совместимости
    (`meridian-core/src/json.rs`). Ничего из перечисленного выше (числа,
    строковые значения, digest/Base64, повторяющиеся ключи значений,
    UTF-16-сортировка, атомарный `ProtocolRoute`, эталонные отпечатки
    плана) этим исправлением не откатывалось.
  - **Исправление (раунд 6): порядок ключей-индексов массива и удаление
    `"__proto__"` — оба расхождения найдены независимым сопоставлением с
    фактическим алгоритмом `canonicalize`/`JSON.stringify`, не заявлены
    заранее как исправленные.** Раунд 5 утверждало, что одной
    лексикографической сортировки `Vec<u16>` по кодовым единицам UTF-16
    достаточно для итогового порядка ключей — это оказалось неточным для
    двух отдельных случаев:
    - **Ключи — индексы массива.** `JSON.stringify` перечисляет
      собственные ключи обычного объекта через ECMA-262
      `OrdinaryOwnPropertyKeys`: сперва все ключи, являющиеся «индексом
      массива» (каноническая десятичная запись целого `0..=2^32-2`, БЕЗ
      ведущих нулей, кроме самого `"0"`), по возрастанию ЧИСЛОВОГО
      значения, и только затем остальные ключи — в ПОРЯДКЕ ВСТАВКИ
      (который для `canonicalize()`'s `out[k] = ...` равен
      лексикографическому порядку UTF-16, так как сама вставка шла в
      порядке `Object.keys(value).sort()`). Контрпример:
      `{"2":2,"10":10}` — канонический текст настоящего
      `checkContentEnvelope` (`"10"` лексикографически МЕНЬШЕ `"2"`, но
      как индекс массива ЧИСЛЕННО больше, поэтому пишется вторым), а
      `{"10":10,"2":2}` (чистый лексикографический порядок, который
      раунд 5 производил) отклонялся как `NotCanonicalJson`. Добавлена
      функция `array_index_value` (`meridian-core/src/json.rs`),
      реализующая именно это определение индекса массива (включая
      границу `"4294967294"` — индекс, и `"4294967295"` = `2^32-1` —
      валидная ДЛИНА массива, но никогда не индекс); `Json::write`'s
      Object-ветка теперь пишет сначала ключи-индексы по возрастанию
      числового значения, затем остальные ключи по `Vec<u16>`.
    - **`"__proto__"` молча отбрасывается, а не переупорядочивается.**
      `canonicalize()`'s `out[k] = canonicalize(value[k])` — обычное
      ECMAScript присваивание свойства; для ЛЮБОГО другого ключа оно
      создаёт собственное свойство на `out`, но `Object.prototype.__proto__`
      — унаследованное СВОЙСТВО-АКСЕССОР (геттер/сеттер, Annex B.2.2.1),
      поэтому присваивание вызывает его СЕТТЕР, а не создаёт собственное
      свойство — вне зависимости от типа присваиваемого значения (сеттер
      либо меняет `[[Prototype]]` самого `out`, когда значение — объект
      или `null`, либо молча ничего не делает для любого другого типа).
      Контрпример: `{"__proto__":1}` — валидный JSON, канонизируется в
      `{}` (собственное свойство `"__proto__"` никогда не появляется на
      `out`), поэтому отклоняется настоящим `checkContentEnvelope` как
      `NotCanonicalJson`, хотя предыдущая ревизия ошибочно принимала его
      (ключ проходил через обычную сортировку и записывался как есть).
      Добавлена функция `is_dunder_proto_key`
      (`meridian-core/src/json.rs`); `Json::write`'s Object-ветка
      отбрасывает такую запись ПЕРЕД разбиением на индексы/остальные
      ключи, на любом уровне вложенности (каждый вложенный `Json::Object`
      применяет фильтр самостоятельно при собственной записи) и
      независимо от типа отброшенного значения (число, `null`, вложенный
      объект). Обычный унаследованный ключ `"constructor"` (ОБЫЧНОЕ, не
      акцессорное свойство `Object.prototype`) этим фильтром не
      затрагивается и продолжает проходить как любой другой ключ —
      проверено отдельным тестом. `Json::Literal` не нуждается в
      аналогичном фильтре: его ключи — заведомо известные во время
      компиляции `&'static str`-идентификаторы Rust-полей, среди которых
      `"__proto__"` никогда не встречается.
    - Оба исправления затрагивают только запись (`Json::write`'s Object
      arm) — `Parser::parse_object` не менялся: и ключ-индекс, и
      `"__proto__"` парсятся как обычные записи `(Vec<u16>, Json)>`,
      фильтрация/переупорядочивание происходит исключительно при
      канонической сериализации, что и требуется для сравнения `content
      === JSON.stringify(canonicalize(JSON.parse(content)))`.
    - Совместимость доказана двенадцатью новыми эталонными тестами через
      публичный `ContentEnvelope` против фактического результата
      настоящего `checkContentEnvelope`
      (`meridian-core/tests/json_canonical_reference.rs`): `{"2":2,"10":10}`
      принят, `{"10":10,"2":2}` отклонён; `"0"` — индекс, `"00"` —
      обычный ключ (ведущий ноль); `"4294967294"` — индекс,
      `"4294967295"` — обычный ключ (граница `2^32-1`); смесь
      числовых/обычных/суррогатных ключей в каноническом порядке принята,
      в произвольном другом порядке — отклонена; `{"__proto__":1}`,
      вложенный `{"a":{"__proto__":1}}` и `{"__proto__":null}` отклонены;
      обычный ключ `{"constructor":1}` принят. Плюс двенадцать новых
      модульных тестов на сами функции `array_index_value`/
      `is_dunder_proto_key` и на запись `Json::Object`
      (`meridian-core/src/json.rs`). Ничего из перечисленного выше
      (числа, строковые значения включая одиночные суррогаты,
      digest/Base64, повторяющиеся ключи, UTF-16-сортировка остальных
      ключей, атомарный `ProtocolRoute`, эталонные отпечатки плана) этим
      исправлением не откатывалось.
  - **Атомарный конструктор `ProtocolRoute` — ключ разрешения равенства
    маршрутов вычисляется, а не принимается произвольной строкой.**
    Предыдущая ревизия этого пакета несла `pub tiebreak_key: String` —
    независимо задаваемое публичное поле, которое ничем не было привязано
    к остальным полям маршрута. Теперь все поля `ProtocolRoute` приватны;
    единственный конструктор — `ProtocolRoute::new(protocol, routed_from,
    source, scope, provenance, mandatory, field_order)`, где `field_order:
    Vec<RouteField>` называет ТОЛЬКО фактический набор полей этого маршрута
    (по `scope`/`provenance`), каждое ровно один раз, в порядке, в котором
    их объявил исходный документ. Конструктор проверяет это соответствие и
    ВЫЧИСЛЯЕТ `tiebreak_key()` из `field_order` и СОБСТВЕННЫХ значений
    полей маршрута через `crate::json::Json::Literal` (сохраняющий заданный
    порядок, как объект-литерал, который Node строит без `canonicalize()`).
    Вызывающая сторона не может передать поля маршрута A вместе с ключом
    маршрута B: такой независимый параметр `tiebreak_key` в публичном API
    больше не существует, а `field_order` определяет только порядок полей
    ЭТОГО ЖЕ маршрута, не подставляет чужие значения. Также исправлена
    диагностика конфликта: число несовместимых обязательных маршрутов —
    `distinctMandatory.length` (число различных имён протоколов), как в
    Node, а не общее число маршрутов-кандидатов. Совместимость с
    Node-эталоном доказана двумя эталонными тестами
    (`meridian-core/tests/resolver_route_reference.rs`), сравнивающими
    вывод `resolve_rules` с фактическим выводом настоящего `resolveRules`
    (`scripts/rule-resolver.mjs`); невозможность независимо подменить поля
    и ключ доказана отдельным отрицательным тестом
    (`tiebreak_key_cannot_be_mismatched_to_a_different_routes_fields`).
  - **Новая прямая зависимость: `sha2 0.11.0` (`default-features = false`).**
    SHA-256 требуется самим предметным алгоритмом (проверка дайджеста нормы
    resolver'ом, каноническая проекция `plan_fingerprint`/`idempotency_key`,
    пересчёт дайджеста `ContentEnvelope`) — собственная реализация SHA-256
    была бы неоправданным риском корректности и безопасности для
    стандартизированного примитива; `sha2` — широко используемая реализация
    RustCrypto без транспортной JSON/YAML функциональности.
  - **Новая прямая зависимость (раунд 4): `ryu 1` (`default-features =
    false`).** Добавлена ПОСЛЕ того, как исполняемое дифференциальное
    сравнение доказало: собственное форматирование Rust `{:e}` не
    реализует округление до чётного ECMA-262 `Number::toString` (см. пункт
    исправления раунда 4 выше) — не гипотетический риск, а
    подтверждённая ошибка корректности. `ryu` — тот же алгоритм
    кратчайшего round-trip форматирования float, которым пользуется сам
    `serde_json`; заменяет только источник цифр, не логику выбора формата.
  - **Процедурное отклонение при первой сдаче пакета (зафиксировано, не
    скрыто).** При подготовке статистики diff исполнитель один раз выполнил
    запрещённую для этой роли операцию записи Git — `git add -N .`
    (intent-to-add); обнаружено сразу и отменено тем же ходом через
    `git reset` без аргументов (вернул индекс к состоянию HEAD; коммит не
    создавался, содержимое рабочего дерева не менялось). Зафиксировано в
    отчёте о передаче пакета как процедурное отклонение, а не молча
    исправлено задним числом; в последующих исправлениях этого пакета
    операций записи Git не выполнялось.
  - Тесты (после раунда 6): 160 модульных тестов в `meridian-core/src`
    (включая детерминированный дифференциальный корпус из 6201 значения для
    `js_number_to_string`, см. выше) плюс 121 интеграционный тест в
    `meridian-core/tests/` (26 `resolver_acceptance.rs`, 35
    `migration_acceptance.rs`, 10 `evidence_acceptance.rs`, 5
    `migration_fingerprint_reference.rs`, 3
    `resolver_route_reference.rs`, 41 `json_canonical_reference.rs`
    (включая 9 добавленных раундом 5 для одиночного/парного суррогата в
    ключе и 12 добавленных раундом 6 для порядка ключей-индексов массива
    и удаления `"__proto__"`), 1 `json_number_corpus_reference.rs`,
    повторно проверяющий тот же корпус через публичный
    `ContentEnvelope`) — области и режимы активации
    resolver'а, независимость от порядка входа, конфликт маршрутов и
    точный текст его диагностики, невозможность подменить ключ разрешения
    равенства маршрутов чужими полями, цикл/неоднозначность `supersedes`
    fail-closed, `requires_reresolution`, разграничение
    `unresolved_items`/`unresolved_applicability` инициативы, неполное
    покрытие/неявное слияние/неверное полномочие/противоречивый
    rollback/неподтверждённый `VERIFIED` плана миграции, чувствительность
    `plan_fingerprint`/`idempotency_key` ко всем нормативно значимым полям
    (проверено как против фиксированных Node-хешей, так и против всех
    полей по отдельности), ложный digest/повреждённый или неканонический
    Base64/неканонический JSON `ContentEnvelope` отклонены (включая
    дробные и экспоненциальные числа, точку округления до чётного,
    дублирующиеся ключи, одиночные/парные суррогаты и порядок ключей вне
    BMP — все против фактического результата настоящего
    `checkContentEnvelope`), невозможность переназначить доказательство
    чужому утверждению, `contradicted`/`inconclusive` никогда не
    подтверждают.
  - **Границы пакета, не расширенные этими исправлениями.** `meridian-core`
    по-прежнему не читает файлы, Git, БД, сеть, переменные окружения и не
    производит вывод; `crate::json` остаётся частным, не экспортируется из
    крейта и не принимает произвольный верхнеуровневый транспортный
    документ — он проверяет каноничность уже полученного,
    предметно-валидного значения (JSON-содержимого одного
    `ContentEnvelope`) и строит канонический вход хешей резолвера и плана
    миграции, а не заменяет собой слой синтаксической проверки
    транспортных данных (`meridian-app`, пакет 4–5).

- **Набор проверки соответствия (`rust-conformance-harness`) — пакет 2
  программы `meridian-rust-migration`.** Независимый от обеих сравниваемых
  реализаций механизм запуска, нормализации и сравнения вердиктов:
  `verification/conformance-harness/conformance-harness.mjs` запускает
  каждую сторону напрямую (без сборки строки для shell), раздельно
  захватывает код завершения, stdout и stderr, разбирает сообщения `FAIL`/
  `WARN` (та же форма, что уже несут `fail`/`warn` в
  `scripts/kernel-validate.mjs`), унифицирует CRLF/LF и сравнивает FAIL и
  WARN как мультимножества без учёта порядка — с сохранением повторов, а не
  дедупликацией. Ошибка запуска, сигнал, отсутствующий код завершения,
  невалидная конфигурация запуска или диагностически выглядящая, но не
  разбираемая строка дают явный отказ харнесса (`harness_error`), а не
  совпадающий или расходящийся вердикт. На этом пакете реальной Rust-
  поверхности ещё нет: механизм проверен на контролируемом корпусе
  (`verification/conformance-harness/fixtures/conformance-harness.fixtures.json`,
  11 случаев) — заведомо совпадающие пары, совпадающие диагностики в разном
  порядке, разные коды завершения, пропавший FAIL, дополнительный WARN,
  смена уровня FAIL/WARN на одном тексте, различие внутри текста
  диагностики, повтор только с одной стороны, ошибка запуска одной стороны
  и намеренно расходящаяся пара, подтверждающая, что харнесс не молчит на
  любом входе. `test/conformance-harness.test.mjs`,
  `.github/workflows/gate.yml` (задача `Kernel validate (synthetic
  instance)`) и `hooks/pre-push` обращаются к одной и той же реализации
  через один и тот же тестовый файл — второго алгоритма сравнения нигде не
  заведено. Харнесс не является командой будущего CLI Meridian, не
  встраивается ни в Node-, ни в будущую Rust-реализацию, не содержит
  предметной логики Meridian и не определяет `Verdict`, `Diagnostic`,
  resolver или порты будущего Rust-домена. Реального сравнения текущего
  Node-валидатора с Rust, предметного Rust-кода и изменений Cargo workspace
  этот пакет не вводит — сравнение реальных Node/Rust-вердиктов наращивается
  начиная с пакета 4 (`rust-source-format-adapters`).
  - **Разделены самопроверка корпуса и публичный код завершения CLI.** Поверх
    одного `runCase` введены две раздельные функции агрегации:
    `runConformanceCheck` (публичная — от неё зависит код завершения CLI;
    читает только фактический статус каждого запрошенного сравнения и никогда
    не читает `expected_status` фикстуры) и `runCorpus` (самопроверка —
    используется только `test/conformance-harness.test.mjs`, сравнивает
    фактический статус с объявленным `expected_status`). До этого исправления
    CLI дал бы код завершения `0` на любом входе, где фактический статус
    совпал с `expected_status` фикстуры, — то есть намеренно расходящаяся пара
    с `expected_status: "divergent"` превращала бы реальное расхождение в
    успешный код процесса. Теперь код завершения CLI — `0` только когда
    каждое запрошенное сравнение фактически `conformant`; любой `divergent`
    или `harness_error` — ненулевой код, независимо от `expected_status`.
    Добавлен настоящий чёрноящичный тест: `conformance-harness.mjs` запускается
    как дочерний процесс, и проверяется его реальный код завершения (не
    внутреннее поле `match`/`status`) на совпадающей, намеренно расходящейся и
    ошибочно запущенной парах. `test/pre-push-git-isolation.test.mjs` дополнен
    записью `test/conformance-harness.test.mjs` в списке файлов, для которых
    строится shim синтетического Kernel, — иначе новый шаг `hooks/pre-push`
    ломал эту существующую регрессию при копировании реального хука в
