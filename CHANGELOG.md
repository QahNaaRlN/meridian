...
- **Объединённая квалификация совместимости рабочих пространств
  (`workspace-compatibility-qualification`) — обязательный участок
  проверки.** Пакет 6, завершающий программу `meridian-workspace-compatibility`:
  продуктово-нейтральный, независимый от хранилища закрытый вердикт
  (`QUALIFIED`/`BLOCKED`/`UNVERIFIED`), композирующий пять уже обязательных
  пакетов программы — `instruction-source-registry`, `controlled-rule-intake`,
  `existing-project-compatibility-mode` и `instance-data-migration` (включая
  его дополнение `instance-canonical-export`) — без дублирования ни одной из
  их проверок. Новые артефакты Ядра:
  `standards/workspace/workspace-compatibility-qualification.md`,
  ...
  `registries/operating-model/fixtures/workspace-compatibility-qualification.fixtures.json`,
  `scripts/lib/workspace-compatibility-qualification.mjs`,
  `test/workspace-compatibility-qualification.test.mjs`.
  - **Ссылки, а не встроенные записи — честная многорепозиторная
    квалификация через `workspace_connection_refs[]`.** `payload` не
    встраивает ни один из составляемых пакетов целиком:
    `workspace_connection_refs` — обязательный, непустой (`minItems: 1`)
    массив закреплённых ссылок, одна на каждое составленное сканирование, а
    не одна привилегированная ссылка — и `migration_plan_ref`/
    `canonical_export_ref` (оба допускают `null`) — все ссылки одной закрытой
    формы `pinned_ref` (`{record_type, id, reference, sha256}`) с
    **обязательным** `sha256` на каждой, разрешаемые через внешнюю границу
    **внутри** проверки и никогда не персистируемые: обнаруженные источники,
    кандидаты правил и их `raw_excerpt`/`normalized_text` не достигают самой
    записи квалификации. Никакие два элемента `workspace_connection_refs` не
    вправе называть один и тот же `id` или `reference` — проверено до попытки
    разрешения. Когда `scope.type` — `project-workspace`,
    `payload.workspace_repository_ids` обязателен и закрыт к точному
    множеству `payload.repository.id`, которое несёт каждый разрешённый и
    сверенный по `sha256` элемент `workspace_connection_refs` — без
    пропусков, лишних записей и дублей; при `repository-scope` поле
    запрещено (схема проверяет оба направления через `allOf`/`if` на
    соседнем `scope`), и сам `workspace_connection_refs` записи
    `repository-scope` несёт **не более одного** элемента (`maxItems: 1`,
    проверено схемой и продублировано библиотекой) — многорепозиторное
    рабочее пространство выражается исключительно через одну запись
    `project-workspace`, а не через несколько записей `repository-scope`
    общего `workspace_id`; доказано самостоятельными позитивным (ровно один
    элемент) и негативным (два элемента) тестами. `sha256` пинует фактически
    пересчитанный дайджест
    содержимого разрешённой записи — `computePlanFingerprint`/
    `computeExportDigest` для плана/экспорта (переиспользованные, а не вторая
    копия) и новая собственная функция этого пакета,
    `computeConnectionDigest`, для каждого сканирования подключения (у
    `existing-project-compatibility-mode.md` нет аналогичной функции для
    переиспользования). Заявленный `sha256`, расходящийся с пересчитанным по
    фактически разрешённой записи, отклоняется **даже когда `id` и
    `reference` совпадают буквально** — бывшая пара `id`/`reference` сама по
    себе не доказывает тождество версии; доказано самостоятельными
    негативными тестами подмены содержимого, включая подмену ровно одного из
    нескольких элементов `workspace_connection_refs`.
    `canonicalConnectionSourceRef` обобщена на закрытое, отсортированное
    множество `id` (`workspace-connection-scan:<id1>+<id2>+...`),
    обратно совместимое с одним `id` — `origin.source_ref` не зависит от
    заявленного порядка сканирований.
  - **Экспорт закреплён за точной версией плана, а не за его `id`.** Проверка
    составленного `canonical_export_ref` вызывает `evaluateInstanceCanonicalExport`
    с **производным** резолвером плана, который эта же проверка строит из уже
    разрешённого и уже сверенного по `sha256` `migration_plan_ref` — а не из
    независимо настраиваемого `resolveMigrationPlan`, который эта проверка
    более не принимает от вызывающего кода. Резолвер, отображающий тот же `id`
    на другое содержимое, структурно не достигает проверки экспорта;
    самостоятельный тест подтверждает это напрямую.
  - **Композиция, а не копия формы.** Каждый разрешённый элемент
    `workspace_connection_refs` — полная запись
    `existing-project-compatibility-mode`, проверяемая **одним** вызовом
    **той же** `evaluateExistingProjectCompatibilityMode` над всеми
    успешно разрешёнными сканированиями сразу; разрешённые
    `migration_plan_ref`/`canonical_export_ref` проверяются **той же**
    `evaluateInstanceDataMigration`/`evaluateInstanceCanonicalExport`. Все
    схемы композиции обязательны независимо от содержимого конкретной записи,
    как и для `existing-project-compatibility-mode`.
  - **Согласованность области.** `scope` записи квалификации обязан полностью
    совпадать со `scope` **каждой** из разрешённых записей — каждого элемента
    `workspace_connection_refs`, а не только первого; разрешённый
    `canonical_export_ref` обязан называть тот же разрешённый
    `migration_plan_ref` и не может присутствовать без него.
  - **Нет тихого `QUALIFIED` поверх решения в ожидании.** Существующий
    приоритет `next_step` (`existing-project-compatibility-mode.md`) не
    заглядывает внутрь `rule_candidates` — только в `findings`. Эта проверка
    закрывает разрыв сама: пока хотя бы один разрешённый кандидат правила
    остаётся `applicability_state: candidate`, `qualification_state` обязан
    быть `UNVERIFIED`, независимо от того, что сообщает `next_step`, и
    независимо от состояния миграции. Доказано на настоящих
    `rule_candidates` (сценарии `duplicate-rule-provenance` и
    `source-change-no-silent-replacement` ниже), а не одной находкой на
    пустом массиве.
  - **Закрытая матрица решений — не то же утверждение, что приёмочные
    сценарии программы.** `payload.qualification_state` — пересчитываемый
    вердикт (`computeQualificationState`), а не заявленное значение: закрытый
    приоритет из девяти табличных строк (восемь логических шагов) читает
    пересчитываемое **множество** `next_step` **всех** разрешённых
    сканирований (любой блокирующий или неопознанный `next_step` на любом из
    них форсирует `BLOCKED`/`UNVERIFIED` для всей записи, независимо от
    заявленного порядка), наличие нерешённого кандидата правила на любом из
    них и, когда план разрешается, его собственный
    `verification.overall_status` и покрытие минченной цели разрешённым
    экспортом. Это внутренняя механика одной функции
    (`workspace-compatibility-qualification.md` §4.1) — отдельное,
    невзаимозаменяемое утверждение от восьми **приёмочных сценариев
    программы** (§4.2: `empty-project-no-instance`,
    ...
    `migration-applicability-preservation`), доказанных отдельными,
    преимущественно не фикстурными тестами на реальных `rule_candidates`,
    `discovered_sources` и, для zero-write и пустого проекта, реальном
    временном каталоге (для zero-write — с `AGENTS.md`/`CLAUDE.md`/правилом
    Cursor; для пустого проекта — подтверждённо без единого созданного
    файла или Instance-каталога) — никогда одной вручную добавленной
    находкой на пустом документе. `multi-repository-workspace` теперь
    компонует **одну** запись квалификации из **двух** реально независимо
    разрешённых сканирований разных репозиториев одного `project-workspace`
    (никогда двух независимых записей квалификации — эта форма отклонена как
    легальная альтернатива, §2): блокирующий конфликт на одном из двух
    форсирует `BLOCKED` для всей записи независимо от чистого состояния
    другого, `next_step: await-owner-decision` на одном из двух форсирует
    `UNVERIFIED`, настоящий нерешённый кандидат правила на одном из двух
    форсирует `UNVERIFIED` даже когда его собственный `next_step` чист,
    повтор `payload.repository.id` между сканированиями и расхождение
    `workspace_repository_ids` с фактически просканированным множеством
    отклоняются — и весь сценарий (шесть тестов) доказан в обоих порядках
    объявления `workspace_connection_refs`, где это применимо. Конфликт
    кандидатов проверен в
    ОБОИХ формах — разрешённый владельцем (один `accepted`, другой
    `not-applicable`) и нерешённый (оба `candidate`, симметричный
    `conflicts_with`, блокирующая находка `conflict`) — в обоих порядках
    объявления, с нормализованным сравнением списка проблем. `blockers`/
    ...
    `UNVERIFIED`).
  - **`migration-applicability-preservation` закрыт на уровне Ядра не
    полностью.** Этот пакет не пересчитывает заново специализированное
    доказательство `instance-data-migration`: сценарий подтверждает Kernel-
    механизм — квалификация принимает `applicability_preservation` только со
    статусом `verified`, подкреплённым **разрешённым через внешнюю границу**
    evidence (`resolveAndCheckEvidence`, `instance-data-migration.mjs`), и
    корректно переводит отсутствие такого подтверждения в `UNVERIFIED`, а не
    в правдоподобную догадку. Тест дополнительно пересчитывает — для
    читаемости фикстуры, не как отдельное Kernel-доказательство —
    **покрытие целей миграции каноническим экспортом** (тот же набор
    `target.id`, что `evaluateInstanceCanonicalExport` уже проверяет своим
    `checkExportCompleteness`), и подтверждает, что потеря, добавление или
    изменение экспортированной записи отклоняются реальным evaluator'ом.
    Фактическое равенство множества применимых норм конкретного продукта до
    и после миграции — предметное, Instance-специфичное утверждение, которое
    этот продуктово-нейтральный Kernel-срез не доказывает и не может
    доказать на синтетических фикстурах; оно остаётся предметом следующего,
    отдельного пакета в Instance-репозитории (Instance field evidence на
    корпусе норм конкретного Instance).
  - **Fail-closed модель безопасности импорта.** Пакет не ослабляет ни одной
    гарантии пяти составляемых контрактов: импортированное содержимое остаётся
    недоверенным, неисполняемым и структурно не персистируется в записи
    квалификации (замкнутая схема отклоняет любое поле, способное нести
    `raw_excerpt` или значение секрета — доказано самостоятельными
    негативными тестами); полномочие не выводится из факта обнаружения или
    чтения; ранее принятое решение владельца, пиновавшее сменившуюся редакцию
    источника, не переносится автоматически на новую редакцию — пиновая
    ссылка перестаёт совпадать с текущим `recorded_state`, и композиция
    отклоняет её, а автоматическая приёмка новой редакции
    (`discovery_status: new` + решённое состояние) отклоняется отдельно;
    секреты не переносятся в доказательства (только непрозрачные ссылки и
    дайджесты); выход за границу источника, неразрешённость внешней ссылки,
    расходящийся `sha256` и неоднозначность классификации или конфликта
    останавливают квалификацию (`BLOCKED`/`UNVERIFIED`, никогда `QUALIFIED`);
    `computeQualificationState` не зависит от порядка объявления полей или
    элементов массива — доказано прямым тестом перестановки реальных
    конфликтующих `rule_candidates`, включая нерешённый конфликт.
  - **Переносимое руководство оператора.** `workspace-compatibility-operator-guide.md`
    документирует пять шагов — scan → classify → owner decision →
    rescan/migration → qualification — без единого продуктового факта.
  - **Интеграция проверки.** `scripts/kernel-validate.mjs` требует схему и
    фикстуры этого контракта безусловно (`FAIL`, не информационный пропуск) и
    вызывает ту же `evaluateWorkspaceCompatibilityQualification`, что и
    `test/workspace-compatibility-qualification.test.mjs`;
    `test/kernel-validate.test.mjs` доказывает, что участок действительно
    блокирует повреждённые данные (снятая схема, повреждённый JSON,
    отсутствующие фикстуры, ложно принятая или ложно отклонённая фикстура).
    Новый самостоятельный тест подключён к `hooks/pre-push` и
    `.github/workflows/gate.yml`.
  - **Граница пакета.** Пакет не сканирует, не разбирает правила, не мигрирует
    и не экспортирует ничего сам; не выбирает постоянное хранилище; не
    включает управляемый режим; не выполняет Git-операций и не изменяет
    `VERSION`.
...
- **Миграция данных Экземпляра (`instance-data-migration`) — обязательный
  ...
  продуктово-нейтральный, независимый от хранилища контракт, который
  позволяет описать, проверить и воспроизводимо применить миграцию записей
  переходного Экземпляра в логические области Meridian
  (`workspace-scope-model.md`). Срез переносит контракт и проверочную
  механику; фактические данные какого-либо конкретного продукта не
  переносятся. Новые артефакты Ядра:
  ...
  - **Нейтральность к хранилищу.** Ни схема, ни библиотека не выбирают Git,
    SQLite, локальную базу или удалённую службу; физический путь не
    становится идентичностью области. Каждое переносимое поле-ссылка
    (`source.repository_ref`, `rollback.*_ref`, `verification.*.evidence_ref`,
    `mapping.merge_rule_ref`) проверяется закрытой проверкой `checkOpaqueRef`:
    корневой POSIX-путь, путь Windows, `file://` URL и личный машинный путь
    отклоняются.
  - **Точный источник.** `payload.source` закрепляет явную редакцию,
    SHA-256-дайджест и `repository_ref`; `qualification: reproducible`
    требует `working_tree_clean: true` на уровне схемы **и** проверяется
    библиотекой против снимка, разрешённого через внешнюю границу
    `resolveSourceSnapshot` (закрытый ответ трансформера, тот же приём, что
    и `resolveSource` в `controlled-rule-intake.mjs`) — заявленное значение
    без разрешённого подтверждения не подтверждает воспроизводимость.
    Грязный, неполный или неоднозначный источник (`not-reproducible`)
    форсирует итоговый вердикт `BLOCKED`, а не более мягкое `UNVERIFIED`.
  - **Полное соответствие.** `record_units` и `mappings` обязаны нести не
    менее одной записи (пустой план миграции недопустим; пустой контейнер
    реестра остаётся легальным). Каждая объявленная единица получает ровно
    одно соответствие (`migrated`, `retained-transitional` или `merged`) —
    пропуск, дублирование и ссылка на необъявленную единицу отклоняются.
    Каждая целевая запись, на которую претендуют `migrated`/`merged`
    mapping, проверяется группой: неявное multi-unit `migrated` без
    `merged` отклоняется; `merged` требует не меньше двух mapping, одно
    общее `merge_rule_ref` и структурно идентичное описание цели у всех
    участников. Классификация не выводится из одного факта совпадения
    `classification_basis` с `unit_ref`.
  - **Сохранение семантики.** Мигрируемая или объединённая цель несёт пять
    фактических полей канонического конверта (`id`, `record_type`, `scope`,
    `origin`, `authority`) и явный `field_basis` (`preserved`/`assigned`)
    для каждого. `target.origin` — новое обязательное поле в форме конверта
    записи (`kind` закреплён `migrated`, `source_ref` проверяется как
    каноническая ссылка на фактически участвующие единицы —
    `deriveOriginSourceRef`); `field_basis.origin` проверяется как всегда
    `assigned` (доперенесённый формат не мог нести это поле). Постоянное
    полномочие цели проверяется закрытой таблицей «одна человеческая
    область — одно полномочие» (та же таблица, что уже действует в
    `controlled-rule-intake.md` §9.1); `delegated-run` — право запуска,
    ведущего миграцию, — не может само назначить постоянное полномочие
    записи, независимо от совпадения `decision_ref`. `target_scope` несёт те
    же структурные ограничения, что и `scope_ref` конверта записи
    (`repository-scope`/`run-state` требуют `workspace_id` и т. д.) — не
    расходящуюся упрощённую копию.
  - **Обратимость.** `payload.rollback` обязателен на каждом плане, и
    обратимость фактически проверяется, а не только закрывается формой схемы.
    `rollback.plan` — **строго взаимоисключающие** варианты:
    ...
    **запрещает** `restoration_evidence_ref`; `verified-restoration` обязан
    ...
    (схема `allOf`/`not`) — посторонняя ссылка другого варианта отклоняется
    схемой, даже если она никогда не разрешается ни одним резолвером, а не
    молча игнорируется как неактивное поле. `source_snapshot_ref` разрешается
    через внешнюю границу `resolveRollbackSnapshot` и обязан назвать тот же
    `repository_ref`, `revision` и полную закрытую форму `digest`, что и
    `payload.source` **этого же плана**, независимо от `source.qualification`;
    конкретный план (`deterministic-reconstruction`'s `deterministic_plan_ref`
    через `resolveDeterministicPlan`, либо `verified-restoration`'s
    `restoration_evidence_ref` через `resolveRestorationEvidence`) обязан
    называть `rollback.source_snapshot_ref`, `id` **и фактически
    пересчитанный `plan_fingerprint`** именно этого плана и, соответственно,
    быть отмеченным применимым либо подтверждающим (`confirms: true`);
    `rewrites_published_history` закреплён буквальным `false`. Простая
    непустая строка — включая заведомо вымышленную (`evidence:invented`,
    `snapshot:invented`) — сама по себе никогда не подтверждает обратимость.
  - **Проверяемое соответствие.** `verification.overall_status: VERIFIED`
    проверяется библиотекой против фактических под-вердиктов покрытия и
    сохранения применимости норм **и** против `source.qualification`
    одновременно. Непустая строка `evidence_ref` сама по себе доказательством
    не является: каждый под-вердикт `verified` проверяется против записи,
    разрешённой через внешнюю границу `resolveEvidence` — закрытый ответ,
    называющий вид доказательства, план, которому оно принадлежит, **и
    фактически пересчитанный `plan_fingerprint` этого же плана** — план с тем
    же `id`, но материально изменённым `mapping`/`target`/`owner_decision`,
    доказательство прежнего содержимого больше не подтверждает — и
    подтверждающий результат (`confirms: true`); вымышленный, неразрешённый,
    относящийся к другому плану/виду, зафиксированный за устаревшим
    содержимым или не подтверждающий ответ отклоняется.
  - **Повторяемость.** `plan_fingerprint` — пересчитываемая
    SHA-256-функция документированной канонической проекции (`source`,
    `record_units`, `mappings`, `rollback`, сортировка по ключу для
    порядко-независимости массивов) — этот же пересчитанный отпечаток
    закрепляет за собой каждое доказательство и ссылка обратимости выше, так
    что материально изменённый план делает недействительным доказательство,
    выданное для его прежнего содержимого, даже при неизменном `id`;
    `idempotency_key` — не произвольная строка, а пересчитываемая функция
    области записи и **канонической идентичности закреплённого источника** —
    `repository_ref`, `revision` и полная закрытая форма `digest`, а не одна
    лишь `revision` — так что два разных исходных репозитория со случайно
    совпавшим текстом `revision` никогда не сталкиваются на одном ключе, а
    повторный прогон над тем же закреплённым входом естественно сталкивается
    на проверке уникальности, а не порождает дубликат под другим ключом.
    `supersedes` проверяется **всегда**, а не только когда предшественник
    удобно оказывается в этом же документе: самоссылка отклоняется;
    предшественник и преемник обязаны совпадать по **полной** области
    (`type`/`id`/`workspace_id`/`organization_profile_id`) и по
    `source.repository_ref`, и преемник обязан пинать **другую**
    `source.revision`. Когда предшественник присутствует в документе, эти
    три сравнения проверяются напрямую; когда нет — ссылка обязана
    разрешиться через внешнюю границу `resolveSupersededPlan` (тот же приём
    закрытого ответа трансформера), и неизвестный или неразрешённый
    `supersedes` **никогда не принимается автоматически**.
  - **Точный источник (уточнение).** Разрешённый снимок дополнительно
    проверяется на совпадение `repository_ref` (снимок, разрешённый для
    другого репозитория, никогда не подтверждает воспроизводимость этого
    источника) и на полную закрытую форму `digest` (лишнее или отсутствующее
    поле внутри `digest` отклоняется), а не только на `revision` и значение
    `digest`.
  - **Интеграция проверки.** `scripts/kernel-validate.mjs` требует схему и
    фикстуры безусловно (`FAIL`, не информационный пропуск) и вызывает ту
    же `evaluateInstanceDataMigration`, что и
    `test/instance-data-migration.test.mjs`; `test/kernel-validate.test.mjs`
    доказывает, что участок действительно блокирует повреждённые данные
    (снятая схема, повреждённый JSON, отсутствующие фикстуры, ложно принятая
    или ложно отклонённая фикстура).

- **Режим совместимости с существующим проектом
  (`existing-project-compatibility-mode`) — обязательный участок проверки.**
  Пакет 4 программы `meridian-workspace-compatibility`: продуктово-нейтральный
  режим подключения существующего проекта через явный ограниченный план
  обнаружения, без единой записи в файлы подключаемого проекта. Новые
  артефакты Ядра: `standards/workspace/existing-project-compatibility-mode.md`,
  ...
  `scripts/lib/existing-project-compatibility-mode.mjs`,
  `test/existing-project-compatibility-mode.test.mjs`.
  - **Нулевая запись в проект, с закрытой файловой границей.** Библиотека не
    предоставляет ни одной операции создания, изменения, удаления или
    переименования файла. `scanDiscoveryPlan` закрывает границу **до** любого
    обращения к диску по слоту: слот обязан пройти `checkPlanLocation`
    (некорректный слот — явное состояние `invalid`, никогда не `missing`);
    `containerRoot` обязан быть непустой строкой; кандидат проверяется на
    строковую принадлежность нормализованному абсолютному корню
    (`path.relative`); затем — уже после того, как что-то существует по этому
    пути (`lstat`, без разыменования конечного звена) — фактически
    разрешённый (`fs.realpathSync`) путь кандидата и корня сравниваются, что
    закрывает и выход через символическую ссылку, ведущую наружу. Только
    после этого выполняются `stat`/`read`. Самостоятельный тест доказывает
    байтовую неизменность временного тестового проекта до и после
    первоначального и повторного сканирования (включая посторонние файлы), а
    отдельные отрицательные тесты — что план `../outside.txt` и символическая
    ссылка из `root` наружу не читают цель и не возвращают её дайджест.
  - **Ограниченное обнаружение с полным взаимоисключающим разбиением
    исходов.** `payload.discovery_plan` — закрытый, явный, ограниченный
    список кандидатов расположения; Ядро не обходит рекурсивно дерево
    проекта. Каждый слот несёт устойчивый `id`, к которому привязаны
    обнаруженный источник, запись о пропаже, запись о нечитаемости и
    находка — расположение вне объявленного плана отклоняется. Для каждого
    объявленного `id` обязан существовать **ровно один** исход среди
    `discovered_sources`/`missing_sources`/`unreadable_sources`: пропущенный,
    задвоенный внутри одного массива и пересекающийся между двумя массивами
    исходы — все три отклоняются одной и той же проверкой числа исходов на
    слот, независимо от позиции в массиве.
  - **Композиция без копирования контрактов, обязательная независимо от
    содержимого.** Обнаруженный источник — полная запись
    `instruction-source-registry`, проверяемая вызовом **той же**
    `evaluateInstructionSourceRegistry` против **той же** схемы; кандидат
    правила — полная запись `controlled-rule-intake`, проверяемая **той же**
    `evaluateControlledRuleIntake`, разрешаемая через границу, построенную из
    обнаруженных источников этого же сканирования. Все четыре схемы
    композиции (`registrySchema`, `envelopeSchema`, `sourceRegistrySchema`,
    `ruleIntakeSchema`) обязательны закрыто **независимо** от того, пусты ли
    `discovered_sources`/`rule_candidates`: отсутствие или неприменимость
    любой из них останавливает всю проверку, а не только ту её часть, что
    касается заполненного массива.
  - **Обнаружение не создаёт решений.** Кандидат с `discovery_status: new`
    (впервые разобранный этим сканированием) обязан остаться `candidate`;
    только `carried-over` вправе нести решённое состояние, и оно всё равно
    заново проверяется полной композицией `controlled-rule-intake`.
  - **Изменение никогда не переписывает молча.** Обнаруженный источник со
    `divergence.status: changed`, пропавший `previously_known` источник и
    любой нечитаемый источник обязаны нести соответствующую запись
    `findings[]`.
  - **`next_step` — одна закрытая приоритетная функция.** Блокирующая находка
    `conflict` требует `resolve-conflict`; иначе блокирующая
    `ambiguous-scope` требует `resolve-ambiguity`; иначе любая другая
    блокирующая находка требует `await-owner-decision`; иначе —
    `continue-compatibility-mode`. Один и тот же набор `findings`, в любом
    порядке, вычисляет ровно одно значение.
...
- **Полевая оценка Meridian (`meridian-field-evaluation`) — обязательный
  ...
  (корректность механизмов, время входа в контекст, возвраты на доработку,
  правильные/ложные остановки, пропущенные нормы, успешность возобновления,
  затраты владельца, дефекты после приёмки), проверяемый журнал наблюдений и
  детерминированно строящийся отчёт — **без свёртки в единый балл**. Новые
  артефакты Ядра: `standards/workspace/meridian-field-evaluation.md`,
  ...
  `scripts/lib/field-evaluation.mjs`, `test/field-evaluation.test.mjs`. Это
  реализация механизма оценки: все фикстуры Ядра явно синтетические, реальных
  измерений продукта пакет не вводит.
  - **Одна схема, два типа записи.** `field-evaluation-observation` (область
    `run-state`, закреплённая ссылка на запуск) и `field-evaluation-report`
    (область `project-workspace`, `workspace_id` — в `payload`, не в `scope`)
    объявляют **одну** специализированную схему; тело выбирает `record_type`
    (`allOf`/`if`/`then`), а не второй файл. `$schema` **разрешается** в
    логическом пространстве имён Meridian — `resolveSchemaRef`/
    `nonPortableReason`/`classifyRevision`/`makeRecordResolver`
    **переиспользованы** из `scripts/lib/context-manifest.mjs`; расходящихся
    копий пакет не вводит.
  - **Восемь характеристик — раздельно.** Каждая имеет закрытый вид измерения
    (классификация / длительность / счётчик), фиксированный по показателю, а
    для классификации — закрытый пул исходов. Классификация требует
    непустого основания (`measurement.basis`); сам факт `blocked`/`passed`
    запуска основанием не является, а исход `unknown` никогда не засчитывается
    как положительный или отрицательный автоматически. Длительность различает
    `calendar` и `active` и не складывает их в одно число; невозможный
    интервал (`end_ts` не после `start_ts`) и отрицательная длительность
    отклоняются. Каждая календарная дата и метка времени контракта
    (`observed_at`, окно наблюдения, отчётный период, `start_ts`/`end_ts`)
    проверяется на **реальное существование** — длину конкретного месяца и
    високосный год, а не только синтаксический шаблон `ГГГГ-ММ-ДД`; 31
    февраля, 31 апреля и невисокосное 29 февраля отклоняются, а не
    округляются молча до ближайшей существующей даты.
  - **Четыре состояния наблюдения, не ноль.** `observed` / `unknown` /
    `not_applicable` / `unmeasurable` — отсутствие наблюдения никогда не
    читается как измеренный нуль; подтверждённый нуль (`observed` со
    значением 0) — законное, отличное от отсутствия значение. Каждый статус,
    кроме `observed`, несёт непустой `status_reason` и не несёт измерения.
  - **Доказательства — закреплённые и разрешаемые через внешнюю границу.**
    `observed` требует минимум одно доказательство, закреплённое точной
    ревизией или `sha256` и **разрешаемое** в закрытый ответ `evidence-result`
    с `observed_result` и `metric_ref` — предметом подтверждения; доказательство
    другого показателя не засчитывается.
  - **Исправление без тихой перезаписи.** Корректировка — новое наблюдение с
    закреплённой ссылкой `supersedes` и непустым `correction_reason`; исходная
    запись не редактируется. Отчёт, включивший обе записи одной цепочки
    исправления, отклоняется как повторный учёт одного факта.
  - **Отчёт: сопоставимость, непересечение, полнота и точный пересчёт при
    любом статусе.** Включённые наблюдения обязаны принадлежать тому же
    рабочему пространству и попадать в отчётный период; повторная ссылка
    отклоняется. `included_observations` и `excluded_observations` держатся
    **непересекающимися** — ни одна ссылка не встречается в обоих списках, ни
    одна пара разных ссылок не разрешается в одно и то же наблюдение поперёк
    списков, и `excluded_observations` не повторяет одну ссылку под разной
    причиной. `per_metric` несёт **ровно восемь** записей — одну на каждую
    характеристику; `sample_observation_ids` каждой обязан **точно**
    совпадать с полным множеством включённых наблюдений этого показателя.
    Исключённое наблюдение показателя обязывает раскрыть исключение в
    `limitations` показателя — скрытое исключение отклоняется. Каждое
    наблюдение, на которое ссылается отчёт, **разрешается и проверяется по
    существу** (`metric_id`, `status`, `measurement`, `workspace_id`,
    `observed_at`, `supersedes_id`, окно и покрытие дефектов) до какого-либо
    использования в агрегации — недействительная или неразрешимая запись даёт
    точную ошибку, а не тихо исчезает. Каждый агрегат `per_metric`
    **пересчитывается** из фактически разрешённой выборки и сверяется
    **точным совпадением** с заявленным **при любом статусе записи** —
    `known`, `unknown` и `not_applicable` больше не освобождены от этой
    сверки, и расходящийся с наблюдениями отчёт отклоняется независимо от
    заявленного статуса.
  - **Доли: числитель и знаменатель всегда, ноль в знаменателе — никогда
    проценты.** Доля классификации несёт `numerator`/`denominator` всегда;
    `percentage` — только при ненулевом знаменателе, вычисляется
    детерминированно (`roundPercentage`). Нулевое число дефектов после
    приёмки при неполном (`partial`/`mixed`) покрытии обязывает непустое
    ограничение, явно называющее, что это не доказательство отсутствия
    дефектов.
  - **Без единого балла.** `FORBIDDEN_PAYLOAD_FIELDS` отклоняет `score`,
    `overall_score`, `grade`, `readiness`, `pass_fail`, `verdict` и подобные —
    на обеих записях.
  - **Детерминированный построитель, не интерфейс.** `buildFieldEvaluationReport`
    — чистая функция: одинаковый набор ссылок и резолвер дают один и тот же
    отчёт независимо от порядка **обоих** входных списков (`includedRefs` и
    `excludedEntries`) по отдельности и вместе — внутренний и итоговый порядок
    канонический (разрешённый `id` наблюдения, `reference` как однозначное
    резервное сравнение). Повтор ссылки и пересечение включённых и исключённых
    **явно отклоняются исключением**, а не устраняются молча через `Map`;
    недействительная или неразрешимая ссылка приводит построитель к закрытому
    завершению с исключением, а не к отчёту, в котором такого наблюдения будто
    не существовало. Построитель разрешает каждую ссылку **тремя раздельными
    шагами** одной границы, ни один из которых не подменяет другой и не
    дублируется второй реализацией: `checkPinnedRef` на самой **входной**
    `pinned_ref` (её собственный `record_type`, устойчивый `id`, переносимая
    `reference` и закрепление — независимо от того, что вернёт резолвер, ведь
    `resolvePinnedReference` сверяет поле пина резолвером только когда оно
    заявлено на входной ссылке и сам по себе не требует его присутствия),
    `resolvePinnedReference` на **разрешённой записи** (её `record_type`,
    замкнутый набор ключей, совпадение `id`/`reference` и закрепления) и
    `resolvedObservationIssues` на предметных значениях. Ранее построитель
    вызывал только второй шаг: ссылка вида `{record_type: "evidence-result",
    reference: "…"}` — без `id`, без `revision`/`sha256` и с неверным
    `record_type` — принималась, если резолвер по `reference` возвращал
    корректное наблюдение; это устранено. Замечание на любом из трёх шагов
    приводит построитель к исключению до вычисления агрегатов. Пара «исходное
    наблюдение + наблюдение, которое его `supersedes`» в `includedRefs`
    **отклоняется построителем исключением немедленно** — без молчаливого
    исключения одного из них из агрегата, без автоматического выбора
    исправления и без возврата отчёта, который лишь затем отклонит
    `evaluateFieldEvaluation`. Отдельного интерфейса, службы или панели
    показателей пакет не вводит.
  - **Реальные именованные фикстуры обходов, не только мутации в тесте.**
    `fixtures/field-evaluation.fixtures.json` несёт **24** записи `valid`,
    **36** записей `invalid` — включая отдельные именованные записи для
    каждого обхода (пересечение включённых/исключённых по ссылке и по
    разрешённому `id`, повтор excluded-ссылки под разной причиной, выдуманный
    агрегат при `unknown`/`not_applicable`, придуманный исход классификации и
    неверный `record_type` в разрешённой проекции) — и отдельную явно
    названную секцию `builder_scenarios` (**8** сценариев) для случаев,
    специфичных для `buildFieldEvaluationReport` (обход границы разрешения
    разрешённой записи, обход границы **самой входной ссылки** — неверный
    `record_type`, отсутствующий `id`, отсутствующие `revision`/`sha256`, и
    их воспроизведённая комбинация, — пара `supersedes`), а не
    самостоятельная запись схемы; второй файл фикстур не создан.
    `test/field-evaluation.test.mjs` читает оба раздела из файла, а не
    собирает эти случаи заново мутациями внутри теста.
  - **Совместимость с существующим журналом показателей.**
    `feedback-and-metrics.md` получил раздел «Stream 3», объясняющий связь
    `validate-log.jsonl` и журнала затруднений с новой моделью: существующий
    журнал — доказательство корректности механизма валидатора и только его;
    остальные семь характеристик остаются `unknown` до отдельного, явного
    решения о сборе реальных данных. Формат и поведение
    `scripts/validate-and-log.mjs` не изменены.

- **Доказательства и передача результата (`evidence-and-handoff`) —
  обязательный участок проверки.** Продуктово-независимый контракт переносимой,
  ограниченной и машинно-проверяемой **передачи состояния и результата одного
  запуска**: что заявлено, что доказано, чем доказано и что делать дальше.
  Новые артефакты Ядра:
  ...
  `scripts/lib/evidence-and-handoff.mjs`, `test/evidence-and-handoff.test.mjs`.
  - **Одна объявленная схема.** Запись объявляет в `$schema` **полную**
    `evidence-and-handoff.schema.json` переносимой относительной ссылкой;
    специализированная схема композирует общий конверт `scoped-record` с телом, а
    `scripts/lib/evidence-and-handoff.mjs` дополнительно прогоняет запись против
    ...
    `scripts/lib/task-specification.mjs` (через `scripts/lib/context-manifest.mjs`),
    закрытые пулы этапов и рабочих состояний — из
    `scripts/lib/execution-state.mjs`, а закрытое правило точной ревизии
    (`classifyRevision`) — из `scripts/lib/context-manifest.mjs`; расходящихся
    копий пакет не вводит.
  - **Область — только `run-state`** (`scope.id` **точно равен**
    `execution_run_ref.id`, `scope.workspace_id` обязателен, `origin.kind:
    built-in` отклоняется).
  - **Детерминированная связь с одним запуском.** `payload` несёт **четыре**
    закрытые структурированные закреплённые ссылки `pinned_ref` — на запуск,
    постановку задачи, запись управления человеком **и манифест контекста**;
    строка вместо объекта и лишнее поле (тело записи) отклоняются;
    `human_control_ref` и `context_manifest_ref` несут `run_id`, равный
    `execution_run_ref.id`, `task_specification_ref` — нет. Тела ссылочных
    записей не встраиваются.
  - **Заявления, утверждения и доказательства — раздельно и проверяемо.**
    `claimed_results`, `verifiable_assertions` и `evidence` — три списка,
    связанных **явно по устойчивым идентификаторам**. **Каждый** элемент
    `evidence` закреплён точной ревизией или `sha256` и **разрешается через
    ...
    `verified` **только** когда покрывающий его элемент разрешился без замечаний,
    его `observed_result` равен `confirmed` **и** преобразователь подтвердил
    связь `covers`; перенос `covers` разрешённого доказательства на постороннее
    утверждение отклоняется, полей `summary` и `covers` из самой передачи
    недостаточно. Обычная строка `reference` без закрепления и разрешения
    подтверждением не является. `unverified` требует причины и не может нести
    `verified`; узкое доказательство не расширяется до необъявленного или
    постороннего утверждения. Статус заявленного результата **выводится с обеих
  ...
  - **Связь с критериями приёмки постановки.** Новое поле
    `payload.acceptance_criteria` **замыкает** список критериев на набор
    ...
    проверяемым утверждением либо `uncovered` с причиной; неизвестный, повторный
    или пропущенный идентификатор отклоняется. Каноническая постановка требует
    **непустой** набор критериев приёмки: закрытый ответ преобразователя с пустым
    ...
    `acceptance_criteria` и `mandatory_checks` не проходит. Самостоятельно
    выбранный пустой `mandatory_checks` при непустом наборе критериев постановки
    отклоняется.
  - **Полный набор `mandatory_checks` замкнут на авторитетный источник.**
    Закрытый ответ преобразователя для `task-specification` расширен минимальным
    машинным представлением ожидаемого набора обязательных проверок (**без тела
    постановки**). Множество `check_ref` передачи сверяется с ним **точно**:
    неизвестная, повторная или **пропущенная** обязательная проверка
    отклоняется — «список непуст» недостаточно, удаление любой обязательной
    проверки из передачи не проходит.
  - **Три состояния обязательной проверки, подкреплённые доказательством
    результата.** `mandatory_checks` различают `passed` / `failed` / `unable`.
    `passed` и `failed` несут `result_evidence_id`, чей разрешённый
    `evidence-result` подтверждает результат (`observed_result` `confirmed` /
    `contradicted`) **и несёт `check_ref` именно этой проверки** — результат
    одной проверки не является результатом другой, перестановка
    `result_evidence_id` между проверками отклоняется; `unable` требует
    проверяемой причины, `result_evidence_id` не несёт и никогда не становится
    успехом. `completed_checks` разрешённой записи `execution-run` подтверждает
    **факт запуска** проверки, а не вердикт: `check_ref` проверки `passed`
    **или** `failed` там присутствует, `unable` — отсутствует. Вердикт
    по-прежнему определяется разрешённым доказательством результата.
  - **Исходные и итоговые ревизии.** `source_state` и `result_state` закреплены
    точными ревизиями по репозиториям (то же закрытое правило) и закрепляют
    **один и тот же набор** `repository_ref`; каждый репозиторий из
    `changed_paths` присутствует в обоих с **различающимся** закреплением.
  - **Раздельные оси событий.** `changed_paths`, `external_effects`,
    `deviations`, `open_gaps`, `required_owner_decisions` и `blockers` — шесть
    раздельных обязательных списков без повторов идентификатора; опущенный ключ —
    ошибка, пустой список — явное утверждение об отсутствии события; множество
    `blockers[].id` равно `blocker_ids` разрешённой записи `execution-run`.
  - **Состояние временного рабочего дерева Git.** `worktree_disposition`
    использует принятый закрытый набор `not_created` / `removed` / `retained`
    (`version-control-flow.md` §5.4); `retained` требует причины, ответственного
    и проверяемого условия очистки, а `not_created` / `removed` этих полей не
    несут. Второй набор состояний не вводится.
  - **Специализированные контракты не дублируются.** Элемент
    `kind: specialised-evidence-record` несёт название специализированного
    контракта и его зафиксированный вердикт, которые **подтверждает разрешённый
    `evidence-result` точным совпадением** — слова самой передачи подтверждением
    не считаются. Общий контракт вердикт **не выводит заново и не
    переопределяет**. Полное тело специализированной записи
    (`parity_comparison`, `baseline`, `post_change_result`, …) в `payload`
    отклоняется.
  - **Внешнее разрешение.** Четыре закреплённые ссылки **и каждый элемент
    `evidence`** разрешаются через границу, внешнюю по отношению к записи
    (резолвер из ключа `resolution` файла фикстур, `makeRecordResolver`), тем же
    **замкнутым контрактом ответа преобразователя**, что у манифеста контекста
    (расширенным полями `acceptance_criteria` (непустой) и `mandatory_checks`
    для `task-specification` и `observed_result` / `covers` / `check_ref` /
    `specialised_contract` / `recorded_verdict` для `evidence-result`). По
    разрешённой записи `execution-run` сверяются `task_specification_ref`,
    `next_step.action` / `next_step.gate`, множество препятствий, состав
    `passed` / `failed`-проверок в `completed_checks` (факт запуска) и правило
    завершающего рабочего состояния; `run-human-control.linked_run_ref` и
    `context-manifest.linked_run_ref` — против закреплённого запуска передачи.
    Без резолвера запись непроверяема и проверка завершается закрыто.
  - **Ограниченность.** Необъявленный избыток контекста (полные тексты файлов,
    журнал команд, `transcript` / `messages` / `chat_history`, выгрузка
    каталога), тела ссылочных записей и поля полевой оценки отклоняются по
    построению. Передача и манифест контекста остаются разными сущностями.
  - **Общий итог — весь запуск.** `outcome.status` — закрытый пул `complete` /
    `blocked` / `handed_off_incomplete`. `complete` означает **весь запуск
    завершён**: разрешённая запись `execution-run` имеет `work_status:
    completed`, `next_step.action` / `next_step.gate` равны `null`, `open_gaps`
    пуст, каждый заявленный результат `established`, каждый критерий приёмки
    `covered` проверенным утверждением, каждая проверка `passed`, нет
    препятствий, блокирующего отклонения и блокирующего требуемого решения
    владельца. Непустое множество препятствий и завершающее рабочее состояние
    `blocked` запуска обязывают `blocked`. Понятие завершения только части
    работы контракт не вводит.
  - **Обязательность.** Контракт — обязательная часть Ядра: отсутствие схемы,
    фикстур или сопутствующего набора `resolution` — `FAIL` общего валидатора
    (участок `evidence-and-handoff-contract`), а не информационный пропуск.
    Обязательность не переключается через `VERSION`. Тот же набор проверок
    выполняет `test/evidence-and-handoff.test.mjs`; он подключён к
    `hooks/pre-push`, к `.github/workflows/gate.yml` и к
    `test/pre-push-git-isolation.test.mjs`. `workflows/task-lifecycle.md`
    указывает `evidence-and-handoff` как форму отчёта при передаче запуска.
