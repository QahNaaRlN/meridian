...
**Status:** `released` · **Version:** [`VERSION`](VERSION) · **Visibility:** private
...
│       workspace/evidence-and-handoff-contract.md ← передача состояния и результата одного запуска
│       workspace/meridian-field-evaluation.md ← практическая оценка: наблюдения и отчёт, без единого балла
...
│   ├── operating-model/ схемы областей, конверта записи, каталога типов задач, постановки задачи, состояния выполнения, ролей и управления человеком, манифеста контекста, доказательств и передачи, полевой оценки, реестра источников
...
│   └── lib/            общие чистые helpers (YAML-subset, JSON-Schema-subset, marked-region reader, task-pattern-catalog, instruction-source-registry, task-specification, execution-state, role-and-human-control, context-manifest, evidence-and-handoff, field-evaluation)
...
node test/evidence-and-handoff.test.mjs              # контракт доказательств и передачи согласован
...
## 5. Добровольные проверки

Все тесты и проверки Kernel сохранены, но с 2026-09-26 запускаются только
добровольно и не блокируют начало работы, приёмку, merge, выпуск или пилот
Concord. `.github/workflows/gate.yml` запускается вручную через
`workflow_dispatch`; `.github/workflows/release.yml` так же остаётся ручным
способом сборки, упаковки и smoke-проверки четырёх целевых платформ.

Локальный `hooks/pre-push` по умолчанию ничего не проверяет и сообщает
`UNVERIFIED`. Полный сохранённый набор включается явно:

```bash
MERIDIAN_RUN_OPTIONAL_CHECKS=1 git push
```

В opt-in режиме настоящий сбой по-прежнему возвращает ненулевой код. Отсутствие
запуска означает `UNVERIFIED`, а не `OK`. Validator, preflight, conformance и
production CLI при прямом вызове не ослаблены. Owner-managed MR и merge-commit
модель остаются действующими независимо от добровольности проверок.

Rust Meridian `0.7.0` выпущен и готов к отдельному
`concord-meridian-onboarding`. Этот onboarding не подменяет продуктовый G0:
его выполнение остаётся следующей самостоятельной работой Concord.
