...
#### 5.24.11. Корректирующий раунд `release-ci-conformance-scope` (2026-09-25)

Статус — **`READY_FOR_ARCHITECT_REVIEW`**; статус выпуска не меняется
(`NOT_RELEASED`). Git write-операций исполнитель не выполнял; Rust-код и
бизнес-контракт не менялись; продуктовые данные Instance в Kernel не
копировались.

Причина: `test/conformance-harness.test.mjs` в строгом режиме требует
`MERIDIAN_INSTANCE` с замороженным источником и без него завершается
ненулевым кодом, поэтому GitHub CI и `hooks/pre-push`, не имеющие приватного
Instance, не могли быть зелёными, не ослабив рубеж.

Решение — два явных режима харнесса:

- обычный запуск без флага остаётся **строгим**: без `MERIDIAN_INSTANCE`
  раздел пакета 8 `meridian-cli-migration` падает, код ненулевой;
- `--kernel-only` выполняет все переносимые проверки, не запускает
  `meridian-cli-migration` над реальным bundle, печатает это как
  `SKIP … UNVERIFIED` и игнорирует заданный `MERIDIAN_INSTANCE`;
  неизвестный аргумент, повтор флага и сочетание с `--test-name-pattern`
  отклоняются кодом 2;
- `.github/workflows/gate.yml` и `hooks/pre-push` запускают `--kernel-only`;
  итоговый notice CI прямо говорит, что GitHub CI не проверяет замороженный
  приватный Instance. `test/pre-push-git-isolation.test.mjs` доказывает, что
  производственный hook передаёт харнессу ровно `--kernel-only` и что без
  этого флага push без Instance блокируется.

**Граница:** зелёная GitHub/`--kernel-only` проверка не является
доказательством пакета 8 и не заменяет строгий real-bundle рубеж. Строгий
запуск без флага с `MERIDIAN_INSTANCE`, указывающим на замороженный источник
с принятым `migration/instance-data`, остаётся отдельным обязательным
release-gate (§5.24.7, §7); `test/instance-fixture` его не заменяет.
