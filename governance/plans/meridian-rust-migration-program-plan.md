...
сохраняет проверенный результат пакета 6, но обязан закрепить роли баз,
эволюцию схемы и событийную границу до реализации командного интерфейса
(§4, §5.4). Ни корректирующий пакет, ни эксперимент этой документальной
синхронизацией не начинаются.
...
| — | Корректирующий пакет: основание знаний и агентной среды (`knowledge-agent-foundation`) | Роли баз `tool`/`workspace`, привязка рабочей базы к редакции Kernel, последовательные миграции схемы, маршрутизация хранилищ, версионируемый конверт наблюдаемого события и отключаемый приёмник событий; спецификации `init`/`doctor`/`export` и импорта согласованы с этими границами | 6, `research-governance-foundation` | planned — ожидает приёмки исследовательского основания (§5.4, §10) |
...
**Критерии приёмки:** общие Rust-ворота §6.1; миграция реальной базы версии 1
в новую версию без потери записей; повторное открытие уже обновлённой базы
идемпотентно; неизвестная будущая версия по-прежнему останавливает работу;
`init`/`doctor`/`export` и будущий импорт имеют однозначную роль базы в
спецификациях; отключённый приёмник событий не меняет вывод, код завершения и
состояние команды; тест запрещает попадание продуктовой записи в базу
инструмента.
...
интегрирован; документальная запись `ready` не считается его началом.
...
- база версии 1 обновляется последовательной атомарной миграцией без потери
  записей, а повторное открытие не повторяет применённый шаг;
- роль базы `tool` или `workspace` и совместимая редакция Kernel читаются
  из метаданных, а не угадываются из пути;
- продуктовая запись отклоняется при попытке направить её в базу инструмента;
- отключённый приёмник наблюдаемых событий не меняет предметный результат,
  stdout, stderr или код завершения операции;
...
обязаны пройти полный набор §6.1–§6.5a, включая §6.4a, одновременно, на полном объёме
...
current_package_status: awaiting_research_governance_acceptance
...
Настоящая ревизия фиксирует локальную приёмку и интеграцию пакета 6 и ставит
перед пакетом 7 корректирующий пакет `knowledge-agent-foundation`. Он не
начинается до отдельной приёмки `research-governance-foundation` и нового
назначения на исполнение. Ни Metis, ни Concord, ни один эксперимент этой
