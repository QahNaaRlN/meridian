...
размещения и порядок публикации в каноническом wiki продукта.
...
### Граница с завендоренным стандартом версионирования
...
Для документа, создаваемого или редактируемого непосредственно в каноническом
wiki, авторитетен завендоренный Kernel-skill
`skills/versioning-standard-docs/SKILL.md`:
...
Он же определяет, нужна ли опубликованной странице собственная версия, правила
SemVer, V12-интерпретацию, историю изменений и обязательную остановку перед
сменой `Status` или версии.

Локальный Front Matter status и статус страницы в каноническом wiki (page
status) — разные поля разных контекстов. Между ними нет автоматического
преобразования: при публикации статус страницы выбирается явно по принятому
skill. При конфликте по статусу или версии страницы приоритет имеет принятый
skill; этот файл продолжает управлять локальным lifecycle и размещением.

В видимой таблице метаданных опубликованной страницы название и контролируемое значение
...
[`templates/template-contract.md`](../templates/template-contract.md); механика конкретной
...
Это правило относится к локальному lifecycle. Для опубликованных страниц используется
...
Запись публикуется в каноническом wiki сразу после написания, без ревью и согласования.
...
### 7.5. Соответствие статуса опубликованной страницы
...
запись остаётся канонической в опубликованном виде и одновременно активным рабочим
...
RFC является публикационным документом. До переноса в канонический wiki локальная
...
| `published`  | Каноническая версия опубликована в каноническом wiki |
...
```markdown
> [!IMPORTANT]
> Документ опубликован в каноническом wiki.
...
### 12.1. Типы, использующие эту же модель

`contract`, `protocol`, `skill`, `template`, `reference` и `changelog` — живые
сопровождаемые артефакты с теми же событиями жизненного цикла, что и `standard`:
их разрабатывают, принимают, поддерживают, выводят из использования и заменяют.
Собственная статусная модель им не заводится, пока не найдётся события, которого
нет в §12; заведённая ради симметрии, она означала бы шесть почти одинаковых
таблиц и шесть мест для расхождения.

`unclassified` — состояние классификации, а не тип, и собственного жизненного
цикла не имеет: документ несёт тот статус, который соответствует его работе, и
поле `unclassified_reason` (см. `document-identity.md`).
...
## 14. Публикация в каноническом wiki

Канонический wiki продукта является каноническим источником для:
...
независимо от канонической версии.

Статус `published` означает «каноническая версия опубликована на площадке,
играющей роль канонического wiki», а не «опубликована в конкретном
инструменте». Определение действует **вперёд**: записи о состоявшихся
публикациях, включая `canonical_url` и `published_at`, задним числом не
переписываются — они фиксируют факт, который действительно произошёл на той
площадке, что была канонической на тот момент.
...
| `published`   | Документ перенесён в канонический wiki      |
...
10. Архивная копия опубликованного документа содержит предупреждение о канонической версии.
