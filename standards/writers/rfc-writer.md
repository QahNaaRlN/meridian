...
Keep an unpublished RFC in `.agent/publication-drafts/rfc/active/`. After a real Confluence publication, add `published`, `published_at`, and `canonical_url`, add the required snapshot notice, and move it to `archive/`. An RFC is not a task plan.

When publishing to Confluence, use `../templates/RFC.confluence-template.md`: do not copy the local H1, and render visible metadata through the unified bilingual table.
