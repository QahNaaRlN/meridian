...
/*
 * ===========================================================================
 * document-identity — the Cargo.toml/Cargo.lock exception (rust-workspace-
 * foundation). Cargo's own manifest and lockfile names carry an upper-case
 * letter and cannot be renamed to kebab-case, in the workspace root as well
 * as in every crate directory. The exception is an exact-string match, not a
 * case-insensitive or pattern-based one — t287 plants a near-miss to prove
 * that.
 * ===========================================================================
 */

// t285 — Cargo.toml and Cargo.lock at the workspace root pass document-identity.
{
  const { kernel, instance } = freshPair('t285');
  write(kernel, 'Cargo.toml', '[workspace]\nmembers = []\n');
  write(kernel, 'Cargo.lock', '# This file is automatically @generated by Cargo.\nversion = 4\n');
  commitAll(kernel);
  check('t285 root Cargo.toml and Cargo.lock pass document-identity', run(kernel, instance), {
    expectExit: 0,
    mustMatch: [/document-identity: \d+ tracked names conform/],
    mustNotMatch: [/document-identity: "Cargo\.toml"/, /document-identity: "Cargo\.lock"/],
  });
}

// t286 — the same names inside a nested crate directory pass too: the
// exception matches the basename wherever it sits, the same way README.md and
// SKILL.md already do.
{
  const { kernel, instance } = freshPair('t286');
  write(kernel, 'crates/widget-core/Cargo.toml', '[package]\nname = "widget-core"\n');
  write(kernel, 'crates/widget-core/Cargo.lock', '# This file is automatically @generated by Cargo.\nversion = 4\n');
  commitAll(kernel);
  check('t286 crate-level Cargo.toml and Cargo.lock pass document-identity', run(kernel, instance), {
    expectExit: 0,
    mustMatch: [/document-identity: \d+ tracked names conform/],
    mustNotMatch: [/document-identity: "crates\/widget-core\/Cargo\.toml"/, /document-identity: "crates\/widget-core\/Cargo\.lock"/],
  });
}

// t287 — a near-miss name (right stem, wrong case) is still rejected: the
// exception must not decay into "anything named Cargo.*" or "upper case is
// fine now".
{
  const { kernel, instance } = freshPair('t287');
  write(kernel, 'crates/widget-core/Cargo.Lock', '# not the real name\n');
  commitAll(kernel);
  check('t287 a near-miss Cargo file name is still rejected', run(kernel, instance), {
    expectExit: 1,
    mustMatch: [/document-identity: "crates\/widget-core\/Cargo\.Lock" is not lower kebab-case/],
  });
}
