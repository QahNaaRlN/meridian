...
Smoke evidence observes a configured runtime action and predicate. It requires the [smoke protocol](smoke-protocol/PROTOCOL.md)'s Test Contract, Execution Context, provenance, evidence set, and §11 computed verdict; the product smoke unit supplies the environments, flows, access and data the run binds to. A manual observation or passing regression test is not a smoke verdict.
